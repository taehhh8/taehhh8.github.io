...
초기에는 **React + JavaScript + Redux Thunk** 기반으로 구현했으나, 초기 로딩·페이지 전환 시 렌더링 지연이 컸습니다. 이후 **Next.js(App Router) + TypeScript + Redux Toolkit**으로 이전하면서 라우트·렌더링 구조 정리, 코드 스플리팅, 타입 안정성을 확보해 체감 성능과 유지보수성이 개선되었습니다.
...
- 비동기 작업은 **createAsyncThunk** 등 Redux Toolkit 표준 패턴으로 처리
